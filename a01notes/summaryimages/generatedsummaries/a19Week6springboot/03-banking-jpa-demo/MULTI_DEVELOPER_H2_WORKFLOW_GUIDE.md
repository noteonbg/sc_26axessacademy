...
### Method 1: Programmatic Seeding via `CommandLineRunner` (Recommended for Dev Teams)
...
* **💡 Golden Pro Tip**: Always check `repository.count() == 0` before saving so restarting the application doesn't insert duplicate records or throw unique key violation exceptions!

---

### Method 2: SQL Scripts via `data.sql` / `import.sql`
...
* **💡 Golden Pro Tip**: Always set `spring.jpa.defer-datasource-initialization=true`. Without this setting, Spring Boot tries to execute `data.sql` before Hibernate creates the tables, causing a `Table "EMPLOYEES" not found` error!

---

### Method 3: Interactive Seeding via H2 Web Console
...
### Method 4: Database Version Control with Flyway (Production Pattern)

For enterprise production setups where schema and seed data are version-controlled:

1. Add `flyway-core` dependency in `pom.xml`.
2. Create versioned SQL scripts in `src/main/resources/db/migration/`:
   * `V1__create_employee_table.sql`
   * `V2__seed_employee_data.sql`
3. Flyway tracks executed scripts in a `flyway_schema_history` table and automatically applies new migrations across team environments.
...
4. **`spring.jpa.hibernate.ddl-auto` Configuration**:
