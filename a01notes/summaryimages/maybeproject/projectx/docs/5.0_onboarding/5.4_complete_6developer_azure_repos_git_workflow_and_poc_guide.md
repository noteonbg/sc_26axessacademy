# 1.0 Quick Terminal Command Cheat Sheet (Commands & Explanations)

## 1.1 Category 1: One-Time Repository Cloning & Setup
- **1.1.1** Command: `git -c http.sslVerify=false clone https://{PAT_TOKEN}@dev.azure.com/AxessAcademy/ProjectX/_git/projectx-ui`
  - Explanation: Clones the repository using PAT authentication with SSL verification bypass for corporate proxies (performed **EXACTLY ONCE** on Day 1).
- **1.1.2** Command: `cd projectx-ui`
  - Explanation: Navigates into the cloned project workspace directory.

## 1.2 Category 2: Fetching & Checking Out Pre-Created Remote Feature Branch (Assigned by Repo Manager)
- **1.2.1** Command: `git fetch origin`
  - Explanation: Connects to Azure Repos and downloads all remote branch references and commits into local hidden memory without altering local working files.
- **1.2.2** Command: `git checkout feature/ui-dev1-auth`
  - Explanation: Switches local workspace to track the assigned pre-created remote feature branch created by Repo Manager.
- **1.2.3** Command: `git pull origin feature/ui-dev1-auth`
  - Explanation: Fetches and integrates any remote updates existing on the pre-created feature branch.

## 1.3 Category 3: Midway Sync When Active Feature Branch Has Changed & Remote "common" Branch (`develop`) Has Changed Due To PR
- **1.3.1** Command: `git add .`
  - Explanation: Stages all local component modifications in your active feature branch.
- **1.3.2** Command: `git commit -m "wip: save local feature progress"`
  - Explanation: Commits local changes so they are safely saved in local git history before rebasing.
- **1.3.3** Command: `git fetch origin`
  - Explanation: Connects to Azure Repos and downloads the latest team commits merged into remote `develop` into local memory (does not touch working files).
- **1.3.4** Command: `git rebase origin/develop`
  - Explanation: Temporarily unhooks your local feature commits, updates your branch base to match the newest `origin/develop` code, and re-applies your commits cleanly on top.
  - **⚠️ CRITICAL SYNTAX ALERT**: Always use `git rebase origin/develop` (with a forward slash `/`). Never write `git rebase origin develop` (with a space), as the space causes Git to checkout local `develop` instead of rebasing your active feature branch!
- **1.3.5** Command: `npm start`
  - Explanation: Launches the local React development server to verify zero compilation or runtime errors after rebasing.
- **1.3.6** Command: `git push origin feature/ui-dev1-auth --force-with-lease`
  - Explanation: Pushes your rebased feature branch back to Azure Repos (`--force-with-lease` safely updates the rebased branch history on the remote server).
...
- **7.4** Step 3 (Rebase Feature Branch onto `origin/develop`):
  - `git rebase origin/develop`  *(MUST use forward slash `/`)*
...
- **9.2** Conceptual Explanation of `git rebase origin/develop` & Syntax Warning:
  ...
  - **MUST USE**: `git rebase origin/develop` (with a forward slash `/`).
  - **DO NOT USE**: `git rebase origin develop` (with a space). Space causes Git to interpret `origin` as the base and `develop` as the branch to checkout, switching you away from your active feature branch!
